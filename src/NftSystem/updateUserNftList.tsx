...
export async function sendMessage(ledger2: any, message: string, recipientId: string, type: string) {
...
export async function UpdateUserNftList(
  ...
  for (var i = unconfirmedTransactionList.length - 1; i >= 0; i--) {
    if (unconfirmedTransactionList[i].recipient === ourContract.ats[0].at && unconfirmedTransactionList[i].sender === nftDistributor) {
      ...
      console.log(nftList);
      var type = ((Math.floor(newArray.length / 8) + 1) ).toString();
      console.log(type);
      sendMessage(ledger2, nftList, ourContract.ats[0].at, type);
